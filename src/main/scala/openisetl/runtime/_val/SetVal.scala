...
case class SetVal(final val value: Set[BaseVal]) 
	...
	implicit private def mkVal(s:Set[BaseVal]) = new SetVal(s)
	...
	override def mul(_rhs : BaseVal) : BaseVal = {
		if (_rhs.isInstanceOf[SetVal]) {
			val rhs = _rhs.asInstanceOf[SetVal]
			return value intersect rhs.value
		}
		super.sub(_rhs)
	}
	
	override def _with(_rhs : BaseVal) : BaseVal = value + _rhs
	
	override def less(_rhs : BaseVal) : BaseVal = value - _rhs
